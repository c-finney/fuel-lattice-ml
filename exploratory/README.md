...
- **`FuelLatticeParameterModelCreation_TrainTest.ipynb`**, a train/test-split sibling
  ...
  between the two notebooks, never reconciled, and another reason this file is not the
...
- **`FuelLatticeParameterUTest.ipynb`**, which trains an `rf1`-equivalent forest for ad hoc
