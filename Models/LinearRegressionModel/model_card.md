...
`Models/binaries/LinearRegressionModel.joblib`, 17,442 bytes, the smallest artifact here.
...
An order of magnitude worse than any ensemble model, which is why it is kept as a baseline.
...
On U(N,C) it has the largest absolute error of the five models and the largest correlation
magnitude, reproducing the size of the compositional dependence while inverting its sign. On
(Ce,Nd)O₂ its slope of 1.030 is the closest to unity of any model here, and almost all of
its 0.9079 Å error is a constant offset.

## Limitations

- **It is not a predictor**, and nothing here is a case for using it as one.
