...
Writes ModelMetrics_CrossVal.csv to METRICS_DIR, and the per-entry out-of-fold
predictions behind it to METRICS_DIR/cv_predictions/<key>.csv. Those per-entry
files are the values plotted in the predicted-vs-actual figures, kept so that
the figures can be rebuilt or re-checked without refitting anything.
...
def evaluate(
    ...
    """
    df, X, Y, _ = prepare_training_frame()
    ...
    for key in keys:
        ...
        _write_cv_predictions(df, Y, Y_pred, cubic_mask, key)
    ...
    _write_model_metrics_json(metrics_df)
    return metrics_df


def _write_model_metrics_json(metrics_df: pd.DataFrame) -> None:
    """
    Mirror each model's rows into Models/<ModelName>/metrics.json.

    These files used to be maintained by hand, and only rf1 ever had one, which
    left four models carrying cross-validation numbers in their cards with no
    machine-readable copy and no guard against drifting from the CSV. Deriving
    them here means the two cannot disagree.
    """
    import json

    for key, group in metrics_df.groupby("model_key"):
        if key not in config.MODEL_FILES:
            continue
        fname, name = config.MODEL_FILES[key]
        model_dir = config.MODELS_DIR.parent / fname.replace(".joblib", "")
        if not model_dir.is_dir():
            print(f"[evaluate]   No directory {model_dir}, skipping metrics.json")
            continue

        payload = {
            "model_key":  key,
            "model_name": name,
            "source":     "Results/metrics/ModelMetrics_CrossVal.csv",
            "method":     "5-fold KFold cross-validation (shuffle, random_state=42), "
                          f"refit per fold via engine.train_models.model_estimators()['{key}']",
            "note":       "Cubic-subset numbers are the ones to cite; see Limitations in "
                          "model_card.md. GENERATED by engine/evaluate_cv.py, do not hand-edit.",
            "params": {
                row["param"]: {
                    k: row[k] for k in
                    ["MSE_all", "MAE_all", "R2_all", "MSE_cubic", "MAE_cubic", "R2_cubic"]
                }
...
def _write_cv_predictions(
    ...
    Y: pd.DataFrame,
    Y_pred: np.ndarray,
    cubic_mask: pd.Series,
    key: str,
) -> Path:
    """
    Persist the out-of-fold prediction for every training entry.

    These are the point-level values behind the predicted-vs-actual figures and
    behind every aggregate in ModelMetrics_CrossVal.csv, so the aggregates can be
    recomputed from this file without refitting. One row per training entry.
    """
    out_dir = config.METRICS_DIR / "cv_predictions"
    out_dir.mkdir(parents=True, exist_ok=True)

    cols = {
        "material_id":         df["material_id"].values,
        "composition_reduced": df["composition_reduced"].values,
        "crystal_system":      df["crystal_system"].values,
        "is_cubic":            np.asarray(cubic_mask),
        "model_key":           key,
    }
    for i, param in enumerate(["a", "b", "c"]):
        cols[f"{param}_true"] = Y[param].values
        cols[f"{param}_pred"] = Y_pred[:, i]

    out_path = out_dir / f"cv_predictions_{key}.csv"
    pd.DataFrame(cols).to_csv(out_path, index=False)
    print(f"[evaluate]   Per-entry predictions saved to {out_path}")
    return out_path
