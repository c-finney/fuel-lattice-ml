...
so its delta is unknown and is reported as such rather than guessed.
...
Every material expands on heating, so `a_exp(T) > a_exp(0 K)`, and thus:
...
**The same-temperature delta is thus MORE POSITIVE than the delta we measured.** That has
...
structure/composition features), and **never a single scalar**, which our own three points already
refute by sign flip; and held-out validation reporting residual + CI. **ICSD is a better source than
COD**, because it records temperature far more consistently.
...
- **MAE vs exp** is NOT pure model error, because it contains the basis mismatch. Do not quote it
...
fit", so `a_true` there is a smooth fitted curve sampled at many compositions, not that many
